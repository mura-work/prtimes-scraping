  ...
  ## 最終行を計算してインスタンス変数に格納する
    ...
    while true do
      ...
      if @sheet[@last_row, 8].present?
        target_data = Company.find_by(company_name: @sheet[@last_row, 1], email: @sheet[@last_row, 4])
        if target_data.present?
          ...
          company =Company.new(
            ...
            charge_employee: @sheet[@last_row, 2],
            tel: @sheet[@last_row, 3],
            email: @sheet[@last_row, 4],
            category: @sheet[@last_row, 5],
            pritimes_url: @sheet[@last_row, 6],
